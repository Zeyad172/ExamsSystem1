...
public class Admin_control {
    static final String DB_URL = "jdbc:mysql://192.168.254.15:3306/nourdb";
    ...
    private void handleAddStudent() throws IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://192.168.254.15:8080/admin/addStudent/"+studentId.getText()+"/"+nameURL+"/"+passwordURL)).GET().build();
...
//(ID, Name, Password, Subject1, Subject2, Subject3)
    private void addProfessor(int id, String name, String password, String s1, String s2, String s3) throws IOException, InterruptedException {
        ...
        s3 = s3.replaceAll(" ",",");
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://192.168.254.15:8080/admin/addProfessor/"+id+"/"+name+"/"+password+"/"+s1+"/"+s2+"/"+s3)).GET().build();
        HttpResponse response = client.send(request,HttpResponse.BodyHandlers.ofString());
        System.out.println("Success");
    }
